...
const listMeasuresController = async (req, res) => {
    const { customer_code } = req.params;
    const { measure_type } = req.query;
    try {
        // Validação de tipo de medição
        const validTypes = ['WATER', 'GAS'];
        if (measure_type && !validTypes.includes(measure_type.toUpperCase())) {
        ...
        const filteredMeasures = measure_type
            ? measures.filter(measure => measure.measure_type.toUpperCase() === measure_type.toUpperCase())
            : measures;
